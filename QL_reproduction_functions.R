...
######
### 201705 EDIT###
          ## Previous versions had a mistake here because QL colonies were producing
          ## drones from their queen's nuclear alleles (when it should have been workers!)
### 201705 EDIT###
######
...
#####
### 201705 EDIT ###
      ## In previous versions, all new colonies had spermathecal contents from the virgin queen. This, however, is only relevant for those new colonies that are QR, as we are assuming that QL coloneis lose their queens during the mating flight (or, at least, she is lost before the colony finds a nest and settles down).
      ## In this new version, those new colonies that become QL are, in effect, clones of the original queen's colony (but with the QL/QR tag switched to QL (col number_alleles+4)). I now first work out whether the colony is going QR or QL; if QR then I copy the data from the old queen's vector but switch the QL/QR tag to QL; if QL then I choose drones and recalculate colony fitness.
## a problem with this is that I no longer have the connection from the new generation to the old generation. So I have to generate another vector above that stores the index (to matrix "population") of the original queen's colony for each row of matrix "new_population"
### 201705 EDIT ###
#####
